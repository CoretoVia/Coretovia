 ...
 * ═══ ЗАЩО СВОЙ ФАЙЛ ═══
 *
 * В MasterBook времето се режеше по ЧЕТИРИ несвързани начина, в четири файла:
 *
 ...
 *
 * Домът е ТУК, и тук е ЕДИНСТВЕНОТО от четирите: `gant.ts` строи решетката ОТ
 * него, а екраните и Книгата режат периода с `koloniNaTakta`. Другите три файла
 * не са пренесени. Давността във филтъра НЕ е такт и, дойде ли, остава своя:
 ...
 * ВЕРИГАТА (`ts · veriga · seq`, ADR-055). Две различни неща с едно име в един
...
/**
 * ПЕРИОДЪТ, КОЙТО КАЛЕНДАРЪТ ПОКАЗВА · от първата до последната му колона.
 *
 * Негово, 13.09 (запис 205), ДОСЛОВНО: „**Да има и според вкарания период, а
 * ако е сега периода да е за периода на такта. Това така да влиае на останалите
 * изчисления и да се съобраазява изцяло в сметките с календара.**"
 *
 * Оттам следва, че обхватът на СМЕТКИТЕ не е отделна настройка, а СЛЕДСТВИЕ от
 * календара: щом колоните са там, там са и парите. Затова периодът се чете от
 * самите колони, а не се смята втори път от такта — инак двете биха се
 * разминали при първата промяна само на едното място (правило 14).
 *
 * `null` при празен календар: липсата е състояние, не нула.
 */
export function periodatNaKolonite(koloni: readonly KolonaNaTakta[]): SvoyPeriod | null {
  const parva = koloni[0];
  const posledna = koloni[koloni.length - 1];
  if (parva === undefined || posledna === undefined) return null;
  return { ot: parva.ot, do: posledna.do };
}

/**
 * Пресича ли се един месец `ГГГГ-ММ` с периода.
 *
 * Сравнява се по МЕСЕЦ, не по ден: редът с пари носи месец задължително и ден
 * по избор (негово, 11.09 · запис 195 т.3). Месец, който само отчасти попада в
 * периода, ВЛИЗА — инак краищата на календара биха изяли пари, които се виждат
 * в него.
 */
export function mesetsatEVPerioda(mesets: string, p: SvoyPeriod): boolean {
  return mesets >= p.ot.slice(0, 7) && mesets <= p.do.slice(0, 7);
}
