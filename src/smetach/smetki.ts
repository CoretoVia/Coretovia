...
export function keshatNaMeseca(o: Ogledalo, mesets: string, kogato: string): Kesh {
  ...
  return {
    ...
    izvlechenie,
    vkarano,
    // ДВЕ СВЕРКИ СТОЯТ НА РЕДА, защото той изброи ДВЕ.
    //
    // Негово, 13.09 (запис 203) т.3 — редът е изброен буквално и в него са:
    // „кеш · дадено ↔ изтеглено: затваря" И „кеш · дадено ↔ вкарано по редовете:
    // разлика". Втората стана ТЪЖДЕСТВО в същия ден, когато даденото почна да се
    // смята от същите редове, срещу които се сверяваше — тя вече не може да
    // покаже друго освен нула.
    //
    // Но нещо, което той е изброил, не изчезва МЪЛЧАЛИВО (правило 12). Тъждество,
    // което винаги затваря, е слаба сверка — и точно затова стои: денят, в който
    // спре да затваря, значи, че даденото и редовете са се разминали, тоест че
    // някой е пипнал едното без другото.
    sverki: [
