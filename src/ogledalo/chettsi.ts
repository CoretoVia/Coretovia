...
export const CHETTSI: Readonly<Record<TipSabitie, Chetets>> = Object.freeze({
  ...
  [TIP.zadachaPotvardena]: (s, st) => {
    ...
    // `zapishi` слива САМО подадените ключове · другите клетки на реда не се пипат
    //
    // И ОЦЕНКАТА СЕ ИЗПРАЗВА · `zadanie/CHISTO/07` И23, ДОСЛОВНО: „Когато
    // Състоянието стане Завършено, **оценката става празна**, редът излиза от
    // дневния ред и отива в архивна таблица." Оценката е матрицата на Айзенхауер
    // — колко бърза и колко важна е задачата. Върху свършена работа тя не значи
    // нищо и само би я държала горе в подредбата.
    //
    // ВРЪЩАНЕТО В РАБОТА не я връща: тя е негов избор, не наш, и се дава наново
    // от човека. Журналът пази старата, тъй че нищо не е изгубено.
    st.tablitsa(p.tablitsa).zapishi(p.id, veriga(s), s.seq, {
      svarshena: p.den === null ? null : { tekst: p.den },
      ...(p.den === null ? {} : { otsenka: null }),
    });
  },
