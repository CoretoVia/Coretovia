/**
 * ЧИСТОТАТА · обход по ВСЕКИ ред и по всяка връзка (И101 т.5).
 *
 * Негови думи: „Направи обстоен и пълен анализ и тест на най-високо ниво за
 * **чистота** и проверка на **всеки ред и групи редове** и **свързаността** на
 * цялата развита и работеща система едновременно, **без празно поле, което да
 * участва**, и **без излишни редове код** и дублирани и грешки, недоопраавени
 * и пропуснати или сбъркани."
 *
 * ═══ ЗАЩО КОМАНДА, А НЕ ДОКЛАД ═══
 *
 * Доклад за чистота остарява в деня, в който някой добави файл. Числата се
 * БРОЯТ, не се оценяват (правило 17) — затова тук е обход, който се пуска
 * (`npm run chistota`) и връща ЧЕРВЕНО, когато прагът се пробие.
 *
 * ═══ ТРИНАЙСЕТТЕ ОБХОДА ═══
 *
 *   1 · МЪРТВО · име, което не се среща никъде освен в декларацията си;
 *   2 · ИЗЛИШЕН `export` · ползва се, но само ВЪТРЕ в своя файл;
 *   3 · САМО ТЕСТ · НИКОЙ не го вика освен тестът. Това НЕ е мъртъв код — то е
 *       непостроена възможност с тест, и разликата е скъпа: обходът за мъртво
 *       веднъж я пропусна, защото ТЕСТ БРОИ ЗА ВИКАЩ (`docs/09` §3);
 *   3б · ИЗНЕСЕНО ЗА ТЕСТА · вика се ВЪТРЕ в своя файл, а `export` е само за да
 *       има тест. Обратното на 3: построено е. Отделено, за да не надува 3;
 *   4 · ПРАЗНО ПОЛЕ, КОЕТО УЧАСТВА · стойност, която може да е празна и въпреки
 *       това влиза в СМЕТКА. Тук се раждат тихите нули;
 *   5 · ИЗЛИШНИ РЕДОВЕ · глътнат отказ (празен `catch`), закоментиран код,
 *       условие, което винаги е вярно, три празни реда подред;
 *   6 · СВЪРЗАНОСТ · всеки файл в `src/` и `app/` има ли поне един внасящ;
 *   6б · САМО ПРЕЗ БАРЕЛА · файл, който производственият код внася САМО през
 *       барел, без нито едно негово име да стига до код — или който внасят САМО
 *       тестовете. Дългът по правило 30, поименно · точен пин (ход 9 · 11.09.2026);
 *   7 · БЕЗ ТЕСТ · файл от домейна, който никой тест не докосва;
 *   8 · ДУБЛИРАНО · еднакви блокове от по няколко реда на две места;
 *   9 · ПЪТ ДО HTML ИЗВЪН ВРАТАТА · `innerHTML` · `insertAdjacentHTML` · `eval`
 *       и `new Function` вън от `app/reshetka/shablon.ts`. Праг НУЛА (резен 6м);
 *   8б · ДУБЛИРАНО ПО СТРУКТУРА · същата ФОРМА под различни имена. Обход 8 не
 *       вижда нищо, което се различава по едно име — а точно такова е скъпото
 *       дублиране тук (резен 6з);
 *  10 · ШУМ · `console.` · `alert(` · `debugger` в `src/` и `app/`. Праг НУЛА,
 *       защото днес там няма нито един: редовният път е `src/yadro/zapis.ts`
 *       (08.09.2026). Прагът ПАЗИ заварено число, не го постига.
 *
 * ═══ ДОКЪДЕ ГЛЕДА · и къде НЕ гледа ═══
 *
 * Само `.ts` (без `.d.ts`). **`.mjs` не се чете изобщо** — тоест `proba/` и
 * самият `stroezh/` са ИЗВЪН всичките дванайсет обхода. Това не е пропуск, който
 * се крие: обход, който мълчи за онова, което не покрива, минава за пълен.
 *
 * Цената се плати на 27.08: `risuvai-ikoni.mjs` внасяше `playwright` вместо
 * `playwright-core` и `npm run ikoni` падаше с „Cannot find package" —
 * обявена команда, която не тръгва. Нито един от тогавашните девет обхода не можеше да
 * я види. Разширяването до `.mjs` е СВОЙ резен: тези файлове нямат типове и
 * половината обходи стъпват на тях.
 *
 * ═══ ПРАГОВЕТЕ ═══
 *
 * Нула за 1, 2, 4, 5, 6 — те са дефекти. „Само тест", „само през барела" и „без
 * тест" са ТОЧНИ ПИНОВЕ (ход 9 · 11.09.2026): състояния, които се обясняват (една
 * възможност чака ход), но не се трупат мълчаливо — нагоре е невикано без ред в
 * дълга, надолу е пин за сваляне с ръка. „Изнесено за теста" се брои и се показва.
 * Праг, вдигнат наслуки, е по-лош от липсващ.
 *
 * ═══ ЧЕТИРИТЕ ПОПРАВКИ НА САМИЯ ОБХОД ═══
 *
 * Обход, който брои лъжливо, е по-скъп от липсващ — затова тук стои какво го е
 * лъгало на първите пускания:
 *
 *   · 143 „мъртви" ИМЕНА, повечето типове. Тип в подписа на изнесена функция
 *     ТРЯБВА да е изнесен, иначе викащият няма как да назове върнатото;
 *   · 10 640 „три празни реда" — изчистването превръща коментарите в празни
 *     редове, а този проект носи дълго обяснение над всяка функция;
 *   · „app/klod.ts не се внася от никъде" — внася се, но с ДИНАМИЧЕН
 *     `import()`: свързващата част се тегли при натискане, не при тръгване;
 *   · „src/yadro/vrata.ts няма тест" — тестовете го внасят през БАРЕЛА
 *     (`src/yadro/index.js`), а обходът търсеше само прякото име.
 */

import { readFileSync, readdirSync, statSync } from 'node:fs';
import { join, relative, basename, dirname, resolve, sep } from 'node:path';
import { fileURLToPath } from 'node:url';

/**
 * ПЪТИЩАТА СЕ ИЗРАВНЯВАТ НА ЕДНО МЯСТО · поправка, платена скъпо.
 *
 * Обход 7 („без тест") се самоизключваше с `if (!f.includes('/src/')) continue;`
 * по средата на файла. На Windows `join` дава `C:\…\src\…` — условието е вярно
 * за ВСЕКИ файл, `continue` се изпълнява винаги, и обходът рапортуваше НУЛА,
 * без да е погледнал нито един файл. Зелено на Ubuntu и зелено на Windows
 * значеха две различни неща, а числата в `docs/03-plan.md` се преписваха от
 * Windows.
 *
 * Затова разделителят се изравнява ВЕДНЪЖ, при събирането; оттам нататък нито
 * един обход не го пипа. Node чете и с права черта на Windows.
 */
function izravni(pat) {
  return pat.split(sep).join('/');
}

// `fileURLToPath`, а НЕ `.pathname` (резен 94 · ADR-152) · виж `chestnost.mjs`.
// `CHISTOTA_KOREN` е за теста на самия обход: пуска командата върху ИЗКУСТВЕНО
// дърво, за да се докаже, че обходите ловят (ADR-015 §7 · обход И).
const KOREN = izravni(
  process.env['CHISTOTA_KOREN'] ?? fileURLToPath(new URL('..', import.meta.url)),
);

/** `--vsichki` показва ЦЕЛИТЕ списъци · без него се съкращават до 40 реда. */
const VSICHKI = process.argv.includes('--vsichki');

// ── събиране на файловете ─────────────────────────────────────────────────

function vsichkiFaylove(papka, sabrani = []) {
  for (const ime of readdirSync(papka)) {
    const pat = izravni(join(papka, ime));
    if (statSync(pat).isDirectory()) {
      if (ime === 'node_modules' || ime === 'dist' || ime.startsWith('.')) continue;
      vsichkiFaylove(pat, sabrani);
    } else if (ime.endsWith('.ts') && !ime.endsWith('.d.ts')) {
      sabrani.push(pat);
    }
  }
  return sabrani;
}

const kod = [...vsichkiFaylove(join(KOREN, 'src')), ...vsichkiFaylove(join(KOREN, 'app'))];
const testove = vsichkiFaylove(join(KOREN, 'tests'));
const prohod = vsichkiFaylove(join(KOREN, 'proba'));

const tekstat = new Map();
for (const f of [...kod, ...testove, ...prohod]) tekstat.set(f, readFileSync(f, 'utf8'));

/**
 * КОЛКО ФАЙЛА Е ВИДЯЛ ВСЕКИ ОБХОД · и защо това е отделно число.
 *
 * Нула находки значи едно от две неща — „чисто е" или „не съм гледал", — а
 * докладът дотук не ги различаваше. Точно това скри мъртвия обход 7. Затова
 * всеки обход СЪОБЩАВА обхвата си и той се печата до броя находки; тестът иска
 * обхватът да е над нула (ADR-015 · обход Й, приложен към самия себе си).
 */
const vidyani = new Map();
function vidyal(obhod, broy, edinitsa = 'файла') {
  vidyani.set(obhod, { broy, edinitsa });
}

const nahodki = [];
function nahodka(obhod, kade, red, kakvo) {
  nahodki.push({ obhod, kade: izravni(relative(KOREN, kade)), red, kakvo });
}

/**
 * Редовете БЕЗ коментари и без низове · ЕДИН четец на лексеми, в реда на файла.
 *
 * Без това всеки обход лови собствените си обяснения: коментар, който описва
 * празен `catch`, се брои за празен `catch`.
 *
 * ═══ ЗАЩО ЕДИН ЧЕТЕЦ, А НЕ ЧЕТИРИ ЗАМЕНИ ═══
 *
 * Дотук изчистването бяха четири последователни регулярни замени (блоков
 * коментар · ред-коментар · \'…\' · "…") и чак НАКРАЯ шаблонните низове. Всяка
 * от тях гледаше файла така, сякаш другите три не съществуват — и точно там се
 * къса:
 *
 *   const red = `<td class="suma">${pishi(x)}</td>`;
 *
 * Заменáта за "…" изяжда `class="suma"` ЗАЕДНО с обкръжаващия текст, шаблонният
 * брояч след нея вече не вижда своята отваряща обратна апострофа — и от този ред
 * НАДОЛУ целият файл минава за „вътре в шаблон", тоест се изтрива.
 *
 * **Измерено, не предположено.** `smetniIzbora` се вика на ред 354 в
 * `app/klaviatura.ts`; обходът виждаше там празен ред и я обявяваше за
 * „непостроена възможност с тест". Същото важеше в различна степен за всеки
 * екранен файл — тоест обходът за чистота беше ПОЛУСЛЯП точно за `app/`, където
 * живеят шаблонните низове.
 *
 * Затова тук има ЕДИН четец с едно състояние, който минава файла отляво надясно
 * и не може да се разсинхронизира: всяка лексема се разпознава от онова
 * състояние, в което наистина се намираме.
 *
 * ═══ КАКВО ОСТАВА И КАКВО ПАДА ═══
 *
 *   · коментари · падат изцяло (иначе обходът чете обясненията си);
 *   · \'…\' и "…" · падат (име в низ не е повикване);
 *   · шаблонен низ · ТЕКСТЪТ пада, `${…}` ОСТАВА. Тук екраните се строят от
 *     шаблони и всяко повикване в тях живее в `${…}`; изтрит целият низ, обходът
 *     обяви `lihvaSDumi` за мъртва в същия ход, в който я закачихме на екрана.
 *     Оставен целият, разметката `class="red smetka"` щеше да мине за повиквания;
 *   · регулярен израз · пада. Разпознава се по предхождащия го знак — иначе
 *     наклонена черта и звездичка ВЪТРЕ в него отварят блоков коментар и пак се
 *     разсинхронизираме.
 *
 * Редовете и стълбовете се пазят: всичко скрито става ИНТЕРВАЛ, не изчезва.
 */

/** След тези знаци „/" започва регулярен израз, не деление. */
const PRED_REGULYAREN = new Set([
  '(',
  ',',
  '=',
  ':',
  '[',
  '!',
  '&',
  '|',
  '?',
  '{',
  '}',
  ';',
  '\n',
  '+',
  '-',
  '*',
  '%',
  '<',
  '>',
  '~',
  '^',
]);

function golKod(tekst, paziNizove = false) {
  let izhod = '';
  let sastoyanie = 'kod';
  /** дълбочината на `${…}` за всеки отворен шаблон · празен стек = не сме в шаблон */
  const shabloni = [];
  let posleden = '';
  let i = 0;

  const skriy = (n = 1) => {
    for (let k = 0; k < n; k++) izhod += tekst[i + k] === '\n' ? '\n' : ' ';
    i += n;
  };
  const pazi = () => {
    izhod += tekst[i];
    if (!/\s/.test(tekst[i])) posleden = tekst[i];
    i += 1;
  };

  while (i < tekst.length) {
    const z = tekst[i];
    const sled = tekst[i + 1];

    if (sastoyanie === 'red-koment') {
      if (z === '\n') sastoyanie = 'kod';
      skriy();
      continue;
    }
    if (sastoyanie === 'blok-koment') {
      if (z === '*' && sled === '/') {
        sastoyanie = 'kod';
        skriy(2);
        continue;
      }
      skriy();
      continue;
    }
    if (sastoyanie === 'edinichen' || sastoyanie === 'dvoen' || sastoyanie === 'regulyaren') {
      const kray = sastoyanie === 'edinichen' ? "'" : sastoyanie === 'dvoen' ? '"' : '/';
      if (z === '\\') {
        if (paziNizove) {
          pazi();
          pazi();
        } else skriy(2);
        continue;
      }
      // Незатворен низ до края на реда е синтактична грешка, не наша грижа —
      // но да НЕ се излиза от състоянието би заслепило остатъка от файла.
      if (z === kray || z === '\n') {
        sastoyanie = 'kod';
        posleden = kray;
      }
      if (paziNizove) pazi();
      else skriy();
      continue;
    }
    if (sastoyanie === 'shablon') {
      if (z === '\\') {
        if (paziNizove) {
          pazi();
          pazi();
        } else skriy(2);
        continue;
      }
      if (z === '`') {
        shabloni.pop();
        sastoyanie = 'kod';
        posleden = '`';
        skriy();
        continue;
      }
      if (z === '$' && sled === '{') {
        shabloni[shabloni.length - 1] += 1;
        sastoyanie = 'kod';
        posleden = '{';
        skriy(2);
        continue;
      }
      if (paziNizove) pazi();
      else skriy();
      continue;
    }

    // ── състояние „код" ──
    if (z === '/' && sled === '/') {
      sastoyanie = 'red-koment';
      skriy(2);
      continue;
    }
    if (z === '/' && sled === '*') {
      sastoyanie = 'blok-koment';
      skriy(2);
      continue;
    }
    if (z === '/' && (posleden === '' || PRED_REGULYAREN.has(posleden))) {
      sastoyanie = 'regulyaren';
      skriy();
      continue;
    }
    if (z === "'") {
      sastoyanie = 'edinichen';
      skriy();
      continue;
    }
    if (z === '"') {
      sastoyanie = 'dvoen';
      skriy();
      continue;
    }
    if (z === '`') {
      shabloni.push(0);
      sastoyanie = 'shablon';
      skriy();
      continue;
    }
    // Затварящата скоба на `${…}` ни връща в текста на шаблона.
    if (z === '}' && shabloni.length > 0) {
      shabloni[shabloni.length - 1] -= 1;
      if (shabloni[shabloni.length - 1] === 0) {
        sastoyanie = 'shablon';
        skriy();
        continue;
      }
    }
    if (z === '{' && shabloni.length > 0) shabloni[shabloni.length - 1] += 1;
    pazi();
  }
  return izhod;
}

const chist = new Map([...tekstat].map(([f, t]) => [f, golKod(t)]));

/**
 * СЪЩИЯТ ФАЙЛ, НО С НИЗОВЕТЕ · само за обхода за дублирано.
 *
 * Останалите обходи търсят ИМЕНА, затова низовете им пречат: „сума" в текст на
 * бутон не е повикване. Дублирането обаче е точно обратното — двата блока са
 * един и същ КОД само ако и съобщенията им съвпадат.
 *
 * Измерено: `app/imoti.ts:580` и `:690` изглеждаха дословно еднакви, а са
 * различни решения — единият казва „Поправката е записана", другият „Наемът е
 * прекратен". Изтрити, двете изречения станаха еднакви празни редове и обходът
 * поиска да ги слеем. Обход, който кара да се слее РАЗЛИЧНО, е по-скъп от
 * липсващ.
 */
const sNizove = new Map([...tekstat].map(([f, t]) => [f, golKod(t, true)]));

// ── 1 и 2 · МЪРТВО и САМО ТЕСТ ────────────────────────────────────────────

const IZNESENI =
  /^export\s+(?:async\s+)?(function|class|const|let|interface|type|enum)\s+([A-Za-z_$][\w$]*)/gm;

/**
 * ТИПЪТ НЕ Е ФУНКЦИЯ · първата поправка на самия обход.
 *
 * Първото пускане обяви 143 „мъртви" имена, и повечето бяха ТИПОВЕ. Това е
 * лъжлива находка: тип, който стои в подписа на изнесена функция, ТРЯБВА да е
 * изнесен — иначе онзи, който вика функцията, няма как да назове онова, което
 * тя връща. TypeScript го иска изрично при декларации.
 *
 * Затова обходът дели двете: изнесен ТИП в публичен подпис е част от лицето
 * на модула; изнесена ФУНКЦИЯ без нито един външен викащ е мъртва.
 */
const TIPOVE = new Set(['interface', 'type', 'enum']);

const imenata = new Map();
for (const f of kod) {
  for (const [, kakvo, ime] of chist.get(f).matchAll(IZNESENI)) {
    imenata.set(ime, { kade: f, tip: TIPOVE.has(kakvo), vKod: 0, vTest: 0 });
  }
}

/** Стои ли този тип на ред, който ИЗНАСЯ нещо — тоест в лицето на модула. */
function vPublichenPodpis(ime, f) {
  return publichniPodpisi(f).some(
    (podpis) =>
      !new RegExp(`^export\\s+(?:async\\s+)?\\w+\\s+${ime}\\b`).test(podpis) &&
      new RegExp(`\\b${ime}\\b`).test(podpis),
  );
}

/**
 * ПУБЛИЧНИТЕ ПОДПИСИ на един файл, всеки СЛЕПЕН в един ред · седмата поправка.
 *
 * Дотук проверката гледаше ЕДИН ред. Подпис на няколко реда я излъга веднага:
 *
 *   export function sgani(
 *     verigi: readonly (readonly Sabitie[])[],
 *     kogato: string,
 *   ): SgunatoOgledalo {
 *
 * Върнатият тип стои на ред, който НЕ почва с `export` — тъй че `SgunatoOgledalo`
 * излезе „излишен export", макар да е самото лице на модула. Обход, който кара
 * да се махне точно онова, което държи подписа, е по-скъп от липсващ.
 *
 * Затова редовете се слепват: от реда с `export` до онзи, който затваря
 * подписа (`{` или `;`). Таванът от 20 реда е спирачка за файл без затваряне —
 * обходът предпочита да пропусне, вместо да увисне.
 */
function publichniPodpisi(f) {
  const redove = chist.get(f).split('\n');
  const podpisi = [];
  for (let i = 0; i < redove.length; i += 1) {
    if (!/^export\b/.test(redove[i])) continue;
    let podpis = redove[i];
    for (let k = 1; k < 20 && !/[{;]\s*$/.test(podpis); k += 1) {
      const sled = redove[i + k];
      if (sled === undefined) break;
      podpis += ` ${sled.trim()}`;
    }
    // ТЯЛОТО НА ИЗНЕСЕН ТИП СЪЩО Е ЛИЦЕ. `RezyumeNaVeriga` стои само вътре в
    // полетата на изнесен `interface` — а онзи, който го получава, трябва да
    // може да назове какво е получил. Затова при тип се чете до затварящата
    // скоба, не до отварящата.
    if (/^export\s+(?:declare\s+)?(interface|type|class|enum)\b/.test(redove[i])) {
      for (let k = 1; k < 200; k += 1) {
        const sled = redove[i + k];
        if (sled === undefined) break;
        podpis += ` ${sled.trim()}`;
        if (/^\}/.test(sled)) break;
      }
    }
    podpisi.push(podpis);
  }
  return podpisi;
}

/** Колко пъти името се среща В СВОЯ файл, без реда на декларацията си. */
function vSvoyaFayl(ime, f) {
  return chist
    .get(f)
    .split('\n')
    .filter(
      (red) =>
        !new RegExp(`^export\\s+(?:async\\s+)?\\w+\\s+${ime}\\b`).test(red) &&
        new RegExp(`\\b${ime}\\b`).test(red),
    ).length;
}

function prebroy(faylove, pole) {
  for (const f of faylove) {
    const t = chist.get(f);
    for (const [ime, zapis] of imenata) {
      if (f === zapis.kade) continue;
      if (new RegExp(`\\b${ime}\\b`).test(t)) zapis[pole] += 1;
    }
  }
}
prebroy(kod, 'vKod');
prebroy([...testove, ...prohod], 'vTest');

/**
 * ТРИТЕ СЪСТОЯНИЯ, и разликата между тях е скъпа.
 *
 *   · **мъртво** · името не се среща НИКЪДЕ освен в собствената си декларация.
 *     Това е дефект: код, който не участва, но се чете, поддържа и мести;
 *   · **излишен `export`** · ползва се, но само ВЪТРЕ в своя файл. Не е дефект,
 *     а обещание за лице на модула, което никой не е поискал. Изнесеният тип
 *     в подписа на изнесена функция ТРЯБВА да е изнесен и не влиза тук;
 *   · **само тест** · НИКОЙ не го вика освен тестът — нито друг файл, нито
 *     собственият му. Това НЕ е мъртъв код (обходът веднъж го обяви така,
 *     защото ТЕСТ БРОИ ЗА ВИКАЩ) — то е непостроена възможност с тест,
 *     `docs/09` §3;
 *   · **изнесено за теста** · вика се ВЪТРЕ в своя файл и отвън само тестът го
 *     стига. Обратното на непостроена възможност: то е ПОСТРОЕНО и жицата му
 *     е вътрешна, а `export` съществува само за да има тест.
 *
 * ═══ ЗАЩО ЧЕТВЪРТОТО СЪСТОЯНИЕ Е ОТДЕЛНО · платено със собствено число ═══
 *
 * Дотук вътрешният викащ не се броеше и четирите имена в `app/redaktsiya.ts` —
 * всяко от които се вика на два реда по-долу в същия файл — влизаха в „само
 * тест". Тоест честният списък на непостроеното носеше в себе си построеното.
 *
 * Това е по-скъпо, отколкото изглежда: този списък е СУРОВИНАТА, по която се
 * тръгва да се търсят обявени, но незакачени възможности (точно така се намери
 * `mozheDaRedaktiraKolona` — правило 23 без нито един викащ). Надут списък
 * значи истинските находки да се крият сред верните случаи, а обход, който
 * праща човека на грешно място, се спира да се чете след третия път.
 */
for (const [ime, z] of imenata) {
  if (z.vKod > 0) continue;
  const vatre = vSvoyaFayl(ime, z.kade);
  if (z.vTest === 0 && vatre === 0) {
    nahodka('1 · мъртво', z.kade, 0, `„${ime}" · не се среща НИКЪДЕ освен в декларацията си`);
  } else if (z.vTest === 0) {
    // ТИП В ПУБЛИЧЕН ПОДПИС НЕ Е ИЗЛИШЕН · шестата поправка на обхода.
    // Шапката го обявяваше отдавна, а кодът го прилагаше само за обход 3.
    // Затова `Tochnost` — типът на втория довод на изнесената `zakragli` —
    // излезе като „излишен export" в мига, в който четецът на лексеми
    // прогледна за `app/`. Тип, който стои на ред с `export`, Е част от лицето
    // на модула: без него викащият няма как да назове онова, което подава.
    if (z.tip && vPublichenPodpis(ime, z.kade)) continue;
    nahodka('2 · излишен export', z.kade, 0, `„${ime}" · ползва се само ВЪТРЕ в своя файл`);
  } else if (!z.tip) {
    if (vatre > 0) {
      nahodka('3б · изнесено за теста', z.kade, 0, `„${ime}" · вика се ВЪТРЕ в своя файл`);
    } else {
      nahodka('3 · само тест', z.kade, 0, `изнесено „${ime}" · вика го САМО тестът`);
    }
  }
}

vidyal('1 · мъртво', imenata.size, 'имена');
vidyal('2 · излишен export', imenata.size, 'имена');
vidyal('3 · само тест', imenata.size, 'имена');
vidyal('3б · изнесено за теста', imenata.size, 'имена');

// ── 3 · ПРАЗНО ПОЛЕ, КОЕТО УЧАСТВА ────────────────────────────────────────

/**
 * Празният низ и `undefined` минават за число в JavaScript: `Number('')` е 0,
 * а `Number(undefined)` е NaN. И двете влизат мълчаливо в сбор.
 *
 * Ловят се ТРИ израза, всеки от които е бил истински дефект в този проект:
 *   · `Number(нещо ?? '')` — празното става нула;
 *   · `parseFloat` и `parseInt` върху поле от форма без проверка;
 *   · `as never` върху сума — то изключва проверката на типа ИЗЦЯЛО и точно
 *     така „undefined" мина за пари и нарисува „NaN €" (`pari.ts`).
 */
const PRAZNI = [
  /**
   * ДЛ-Т9 · ТЪРСИ СЕ ИЗЧИСТЕНАТА ФОРМА (ход 9 · 11.09.2026).
   *
   * Дотук изразът искаше `?? ''`, а `chist` е текстът, от който `golKod` е скрил
   * всеки низ заедно с кавичките му — тоест `''` там никога не съществува и
   * изразът беше СТРУКТУРНО МЪРТЪВ: живият случай `Number(klyuch.split('#')[2] ?? '')`
   * в `src/kniga/chetene.ts` минаваше зелен. Първата поправка (четене на текста
   * с низовете) също не хващаше: и там отварящата кавичка се крие. Затова се
   * търси ИЗЧИСТЕНАТА форма — `??`, после само интервали, после `)` — а тя значи
   * точно „резервата беше НИЗ" (`''` · `""` · `'0'`), всяка от които е число от низ:
   * тиха нула или NaN. Едно ниво вложени скоби, за да мине през `split('#')`.
   * Обход, чийто израз не може да съвпадне, не е обход.
   */
  [
    /Number\(\s*(?:[^()\n]|\([^()\n]*\))*\?\?\s*\)/g,
    "Number(… ?? '‹низ›') · резервата е низ · празното става НУЛА мълчаливо",
  ],
  [/\bparseFloat\s*\(/g, 'parseFloat · празното дава NaN и влиза в сбор'],
  // `parseInt` БЕЗ основа · с основа (8 · 16) той чете вече проверен низ и е
  // верен. Първото пускане обяви и двата верни случая — обход, който брани
  // правилното, кара човек да го „поправи" в грешно.
  [/\bparseInt\s*\([^,)]*\)/g, 'parseInt без основа · „08" става 0 на стари четци'],
  [/_st[^\n]*\bas never\b/g, 'сума през „as never" · проверката на типа пада ИЗЦЯЛО'],
  // ЗАКРЪГЛЯНЕТО НА ПАРИ ИМА ЕДИН ДОМ (резен 6и · правило 3).
  //
  // `deliZakragleno` живее в `yadro/pari.ts` и дълго имаше ЕДИН викащ, докато
  // същата операция беше пренаписана на четири места с `Math.round` — и точно
  // там, където числото застава пред него (Продажби · Калкулаторът). Двете НЕ
  // са едно: `Math.round(-2.5)` е -2, а половинката тук се закръгля ОТ нулата.
  // Сторното е обърнат знак, тъй че разликата излиза на сверка, която не затваря.
  [
    /Math\.round\s*\([^\n]*_st\b/g,
    'Math.round върху пари · домът на закръглянето е `deliZakragleno` (правило 3)',
  ],
];

for (const f of kod) {
  const redove = chist.get(f).split('\n');
  redove.forEach((red, i) => {
    for (const [izraz, duma] of PRAZNI) {
      izraz.lastIndex = 0;
      if (izraz.test(red)) nahodka('4 · празно поле', f, i + 1, duma);
    }
  });
}

vidyal('4 · празно поле', kod.length);

// ── 4 · ИЗЛИШНИ РЕДОВЕ ────────────────────────────────────────────────────

for (const f of kod) {
  const tekst = chist.get(f);

  /**
   * ГЛЪТНАТ ОТКАЗ · `catch` без НИТО ЕДИН ред и без НИТО ЕДНА дума.
   *
   * Търси се в СУРОВИЯ текст. Първото пускане обяви 12 находки и всичките
   * дванайсет бяха обяснени — `catch { // частен прозорец: пак работи, само не
   * помни }`. Изчистването маха коментара и обяснението изглежда като мълчание.
   *
   * Точно това е разликата, която този проект пази: отказ, КАЗАН с дума, не е
   * глътнат. Глътнат е онзи, който не оставя нищо.
   */
  for (const m of tekstat.get(f).matchAll(/catch\s*(?:\([^)]*\))?\s*\{\s*\}/g)) {
    nahodka(
      '5 · излишен ред',
      f,
      tekstat.get(f).slice(0, m.index).split('\n').length,
      'глътнат отказ · `catch {}` без ред и без дума',
    );
  }

  // УСЛОВИЕ, КОЕТО ВИНАГИ Е ВЯРНО.
  for (const m of tekst.matchAll(/\bif\s*\(\s*(?:true|1)\s*\)/g)) {
    nahodka(
      '5 · излишен ред',
      f,
      tekst.slice(0, m.index).split('\n').length,
      'условие, което винаги е вярно',
    );
  }

  // ЗАКОМЕНТИРАН КОД · ред, който е бил код, а сега е коментар. Търси се в
  // ЦЕЛИЯ текст (не в изчистения) и само при явни белези: коментар, който
  // завършва с „;" или „{", тоест изречение на код, не на български.
  tekstat
    .get(f)
    .split('\n')
    .forEach((red, i) => {
      const t = red.trim();
      if (!t.startsWith('//')) return;
      const tyalo = t.slice(2).trim();
      if (/[;{]$/.test(tyalo) && /[A-Za-z_$][\w$]*\s*[=(.]/.test(tyalo)) {
        nahodka('5 · излишен ред', f, i + 1, `закоментиран код · „${tyalo.slice(0, 48)}"`);
      }
    });

  /**
   * ТРИ ПРАЗНИ РЕДА подред · нито дял, нито дъх.
   *
   * Брои се в СУРОВИЯ текст, не в изчистения. Първото пускане обяви 10 640
   * находки, защото изчистването превръща коментарите в празни редове — а този
   * проект носи дълги обяснения над всяка функция. Обход, който брои
   * собственото си обяснение за боклук, е по-лош от липсващ.
   */
  const surovi = tekstat.get(f).split('\n');
  for (let i = 2; i < surovi.length; i += 1) {
    if (surovi[i].trim() === '' && surovi[i - 1].trim() === '' && surovi[i - 2].trim() === '') {
      nahodka('5 · излишен ред', f, i + 1, 'три празни реда подред');
    }
  }
}

vidyal('5 · излишен ред', kod.length);

// ── КАРТАТА НА ВНОСА · един индекс, два обхода ────────────────────────────

/**
 * КОЙ КОГО ВНАСЯ · по РАЗРЕШЕН ПЪТ, не по базово име.
 *
 * Дотук обходи 6 и 7 търсеха `/‹базово име›.js'` в слепения текст. Единайсет
 * базови имена се повтарят в `src` и `app` (`dumi` · `imoti` · `modeli` ·
 * `nastroyki` · `obshto` · `prodazhbi` · `shema` · `sluzhiteli` · `smetki` ·
 * `tablitsa` · `upravlenie`), тъй че тест, който внася `smetach/smetki.js`,
 * обявяваше за тестван и `komandi/prozortsi/smetki.ts`. Четири файла минаваха
 * така безплатно.
 *
 * Затова всеки специфер се РАЗРЕШАВА спрямо своя файл и `.js` се връща към
 * `.ts` — съвпадението става точно и еднозначно.
 *
 * ДИНАМИЧНИЯТ ВНОС Е ВНОС. `kniga/ooxml.js` се тегли при НАТИСКАНЕ, за да не
 * влачи 258 KB при тръгване — обход, който брои само `from '…'`, го обявява за
 * несвързан и праща човек да трие работещо мързеливо теглене.
 */
const SPETSIFER = /(?:from|import\()\s*'([^']+)'/g;

/**
 * ВНОС В НИЗ НЕ Е ВНОС (ход 9 · 11.09.2026).
 *
 * Тестовете, които доказват машините върху счупено дърво (`tests/chistota.test.ts` ·
 * `tests/sloevete.test.ts`), носят цели файлове като низове: `"import { Izpalnitel }
 * from '../src/porta/izpalnitel.js';"`. Четен суров, такъв ред обявяваше
 * `izpalnitel.ts` за тестван и обход 7 слизаше без причина. Специферите в кода са
 * винаги в единични кавички (`stil` го пази), тъй че двойните и шаблонните низове
 * се скриват, преди да се търси `from '…'`.
 */
function bezChuzhdiNizove(t) {
  return t
    .replace(/"(?:[^"\\\n]|\\.)*"/g, (m) => ' '.repeat(m.length))
    .replace(/`(?:[^`\\]|\\.)*`/g, (m) => m.replace(/[^\n]/g, ' '));
}

/**
 * ТИПОВИЯТ ВНОС НЕ ТЕСТВА (ход 9 · 11.09.2026). `import type { X } from '…'` изчезва при
 * компилация и не изпълнява нито ред от файла — тест или помощник, който внася само
 * тип, не е докоснал кода. За свързаността (обход 6) типът Е връзка; за „без тест"
 * (обход 7) не е. Затова четенето има две лица.
 */
function vnesenoOt(fayl, bezTipove = false) {
  const nabor = new Set();
  let tekst = bezChuzhdiNizove(tekstat.get(fayl));
  if (bezTipove) tekst = tekst.replace(/import\s+type\s+[^;]*?from\s*'[^']+'/g, ' ');
  for (const [, spets] of tekst.matchAll(SPETSIFER)) {
    if (!spets.startsWith('.')) continue;
    nabor.add(izravni(resolve(dirname(fayl), spets)).replace(/\.js$/, '.ts'));
  }
  return nabor;
}

const vnasya = new Map();
for (const f of [...kod, ...testove, ...prohod]) vnasya.set(f, vnesenoOt(f));

// ── 5 · СВЪРЗАНОСТ · файл, който никой не внася ───────────────────────────

const VHODNI = new Set(['main.ts', 'index.ts', 'izdanie.ts', 'vite-okolna-sreda.d.ts']);

const vnesenite = new Set();
for (const [f, nabor] of vnasya) for (const cel of nabor) if (cel !== f) vnesenite.add(cel);

let vidyaniSvarzanost = 0;
for (const f of kod) {
  if (VHODNI.has(basename(f))) continue;
  vidyaniSvarzanost += 1;
  if (!vnesenite.has(f)) {
    nahodka('6 · несвързан', f, 0, 'файлът не се внася от НИКЪДЕ');
  }
}
vidyal('6 · несвързан', vidyaniSvarzanost);

// ── 5б · СВЪРЗАНОСТ ПРЕЗ БАРЕЛА · имена, не файлове ───────────────────────

/**
 * ОБХОД 6 ГЛЕДАШЕ ФАЙЛОВЕ, НЕ ИМЕНА (ход 9 · 11.09.2026 · планът от 10.09, 2.6).
 *
 * Барелът `src/yadro/index.ts` изнася наново единайсет файла, тъй че всеки от
 * тях беше „свързан" — дори когато нито едно негово име не стига до `src/` или
 * `app/` (`zapis.ts` · `data.ts`). И тестът броеше за внасящ: файл, който само
 * тестовете внасят, минаваше за жив (`hash-node.ts` · `naivno.ts`).
 *
 * Тук файлът е находка, когато (А) производствен код го внася САМО през барел и
 * нито едно от изнесените му имена не се ВНАСЯ от барела в друг производствен файл,
 * или (Б) го внасят САМО тестове и проходът. Имената се четат от самите
 * `import { … } from` (не от текста: `NIVA` е изнесено и от `nahodki-nap.ts`, и
 * текстово съвпадение би броило чуждото име за свое). Прагът е ТОЧЕН пин (както 3
 * и 7): това е дългът по правило 30, поименно, и той може само да пада — всеки файл
 * тук има или викащ в следващите ходове (ред в `docs/registar-na-dalga.json`), или
 * дом в `tests/`, или го няма. Един скок през барела, не верига (както обход 7).
 */
const kodNabor = new Set(kod);
const eBarel = (f) => basename(f) === 'index.ts';
const otnositelen = (f) => izravni(relative(KOREN, f));
const vnasyashtiOtKod = new Map();
const vnasyashtiOtTest = new Map();
for (const [f, nabor] of vnasya) {
  for (const cel of nabor) {
    if (cel === f) continue;
    const kade = kodNabor.has(f) ? vnasyashtiOtKod : vnasyashtiOtTest;
    if (!kade.has(cel)) kade.set(cel, new Set());
    kade.get(cel).add(f);
  }
}
/** кои имена внася всеки производствен файл и откъде · `import { a, b as c } from '…'` */
const IMENA_V_IMPORT = /import\s+(?:type\s+)?\{([^}]*)\}\s*from\s*'([^']+)'/g;
const vneseniImena = new Map();
for (const d of kod) {
  const poIzvor = new Map();
  for (const [, spisak, spets] of tekstat.get(d).matchAll(IMENA_V_IMPORT)) {
    if (!spets.startsWith('.')) continue;
    const izvor = izravni(resolve(dirname(d), spets)).replace(/\.js$/, '.ts');
    if (!poIzvor.has(izvor)) poIzvor.set(izvor, new Set());
    for (const chast of spisak.split(',')) {
      const ime = chast
        .replace(/^\s*type\s+/, '')
        .trim()
        .split(/\s+as\s+/)[0]
        ?.trim();
      if (ime) poIzvor.get(izvor).add(ime);
    }
  }
  vneseniImena.set(d, poIzvor);
}
const imenataNa = new Map(
  kod.map((f) => [f, [...chist.get(f).matchAll(IZNESENI)].map((m) => m[2])]),
);
let vidyaniPrezBarela = 0;
for (const f of kod) {
  if (VHODNI.has(basename(f)) || !vnesenite.has(f)) continue;
  vidyaniPrezBarela += 1;
  const otKod = vnasyashtiOtKod.get(f) ?? new Set();
  if ([...otKod].some((v) => !eBarel(v))) continue; // пряк производствен внос · свързан
  if (otKod.size === 0) {
    const testove = [...(vnasyashtiOtTest.get(f) ?? [])].map(otnositelen).join(' · ');
    nahodka(
      '6б · само през барела',
      f,
      0,
      `внасят го САМО тестове (${testove}) · нито един производствен файл`,
    );
    continue;
  }
  const imena = new Set(imenataNa.get(f) ?? []);
  const stiga = kod.some((d) => {
    if (d === f || eBarel(d)) return false;
    const poIzvor = vneseniImena.get(d);
    return [...otKod].some((barel) =>
      [...(poIzvor.get(barel) ?? [])].some((ime) => imena.has(ime)),
    );
  });
  if (!stiga) {
    const spisak = [...imena];
    const pokazani = `${spisak.slice(0, 4).join(' · ')}${spisak.length > 4 ? ' …' : ''}`;
    nahodka(
      '6б · само през барела',
      f,
      0,
      `внася го САМО барелът (${[...otKod].map(otnositelen).join(' · ')}) · нито едно от ${spisak.length} негови имена (${pokazani}) не се внася оттам в код`,
    );
  }
}
vidyal('6б · само през барела', vidyaniPrezBarela);

// ── 6 · БЕЗ ТЕСТ ──────────────────────────────────────────────────────────

/**
 * БАРЕЛЪТ БРОИ. `src/yadro/index.ts` изнася наново Вратата, Журнала и
 * останалите — тест, който внася от него, ТЕСТВА тях. Обход, който търси само
 * прякото име, обяви девет тествани файла за нетествани.
 *
 * Един скок, не верига: тест → барел → файл. Пълната достижимост би обявила за
 * тестван всеки файл, до който се стига отнякъде — тоест почти всичко.
 */
const dokosnatiOtTest = new Set();
for (const t of [...testove, ...prohod]) {
  for (const cel of vnesenoOt(t, true)) {
    dokosnatiOtTest.add(cel);
    if (basename(cel) === 'index.ts' && vnasya.has(cel)) {
      for (const prez of vnasya.get(cel)) dokosnatiOtTest.add(prez);
    }
  }
}

/**
 * ГЛЕДА И `app/`, не само `src/`.
 *
 * Дотук обходът се ограничаваше до домейна. Но екранът е най-слабо
 * покритата част на хранилището и точно там числото трябва да се вижда.
 * Обход, който гледа половината код и мълчи за другата, е по-малката форма на
 * същия дефект. Числото е СЪСТОЯНИЕ (няма праг), а не дефект.
 */
let vidyaniBezTest = 0;
for (const f of kod) {
  if (VHODNI.has(basename(f))) continue;
  vidyaniBezTest += 1;
  if (dokosnatiOtTest.has(f)) continue;
  nahodka('7 · без тест', f, 0, 'нито един тест не внася този файл');
}
vidyal('7 · без тест', vidyaniBezTest);

// ── 7 · ДУБЛИРАНО · еднакви блокове ───────────────────────────────────────

/**
 * Търсят се блокове от ПЕТ смислени реда, повторени дословно на две места.
 * Пет е границата, под която съвпадението е случайно (три реда затваряне на
 * скоби се повтарят навсякъде и не значат нищо).
 *
 * ═══ СПИСЪК НЕ Е ЛОГИКА · третата поправка на обхода ═══
 *
 * Първото пускане обяви 67 „дублирани" блока. Почти всички бяха СПИСЪЦИ —
 * съседни редове от списък с внасяния или полета на интерфейс. Два файла,
 * които внасят едни и същи седем имена, не дублират логика; те ползват един
 * и същ модул, което е точно каквото трябва.
 *
 * Затова блок се брои само когато носи ДЕЙСТВИЕ: поне два от петте реда трябва
 * да са изречения — повикване, присвояване, връщане или условие.
 */
const DEYSTVIE = /(?:\breturn\b|\bif\s*\(|=>|[\w$)\]]\s*\(|[^=!<>]=[^=])/;

const BLOK = 5;
const blokove = new Map();
for (const f of kod) {
  const redove = sNizove
    .get(f)
    .split('\n')
    .map((r, i) => ({ tekst: r.trim(), red: i + 1 }))
    .filter((r) => r.tekst !== '' && r.tekst !== '}' && r.tekst !== '});' && r.tekst !== '{');
  for (let i = 0; i + BLOK <= redove.length; i += 1) {
    const parcheta = redove.slice(i, i + BLOK);
    const klyuch = parcheta.map((r) => r.tekst).join('\n');
    if (klyuch.length < 120) continue;
    if (parcheta.filter((r) => DEYSTVIE.test(r.tekst)).length < 2) continue;
    const veche = blokove.get(klyuch);
    if (veche) {
      if (veche.kade !== f || Math.abs(veche.red - redove[i].red) > BLOK) {
        nahodka(
          '8 · дублирано',
          f,
          redove[i].red,
          `${BLOK} реда, еднакви с ${relative(KOREN, veche.kade)}:${veche.red}`,
        );
      }
    } else {
      blokove.set(klyuch, { kade: f, red: redove[i].red });
    }
  }
}

vidyal('8 · дублирано', kod.length);

// ── 8б · ДУБЛИРАНО ПО СТРУКТУРА · еднаква ФОРМА, различни имена ───────────

/**
 * ОБХОД 8 ЛОВИ ДОСЛОВНОТО. ТОЗИ ЛОВИ ФОРМАТА.
 *
 * Обход 8 иска ПЕТ дословно еднакви реда. Той брои нула — и числото е вярно, —
 * но мери малко: всяко дублиране, което се различава по ЕДНО име, му е
 * невидимо. А точно такова е скъпото дублиране в този проект: два записа от
 * форма с различен префикс на белега, две фабрики за команда-близнак, осем
 * писача на лист, които повтарят „инструкции · лента · глава · данни".
 *
 * Затова тук редовете се НОРМАЛИЗИРАТ, преди да се сравнят: всяко име става
 * `N`, всеки низ — `S`, всяко число — `0`, интервалите падат. Остава СКЕЛЕТЪТ.
 *
 * ═══ ЗАЩО ПЕТ РЕДА И КЛЮЧ ≥ 80 ═══
 *
 * Мерено с ръка върху цялото хранилище: при четири реда и по-къс ключ броят
 * скача на двайсет и повече, и почти всичко е шум (`if` · `return` · извикване).
 * Праг, който вика при всяко второ извикване, се изключва от човек в първата
 * седмица — и тогава обходът е по-лош от липсващ (ADR-051).
 *
 * При пет реда и ключ от поне осемдесет знака остават ТРИ места, и трите са
 * истински. Числото е мерено, не избрано наслуки.
 *
 * ═══ ПРОЗОРЦИТЕ СЕ СЛИВАТ ═══
 *
 * Плъзгащият прозорец обявява един и същ блок по веднъж за всяко свое
 * отместване — три находки за едно място. Затова съседните се сливат: доклад,
 * който брои едно нещо три пъти, кара човек да не му вярва.
 */
const NORMALIZIRAY = (red) =>
  red
    .trim()
    .replace(/'[^']*'|"[^"]*"|`[^`]*`/g, 'S')
    .replace(/\b[A-Za-z_$][\w$]*\b/g, 'N')
    .replace(/\b\d[\d_]*\b/g, '0')
    .replace(/\s+/g, '');

const REDA_V_PROZORETSA = 5;
const NAY_KASIYAT_KLYUCH = 80;

const skeleti = new Map();
const strukturni = [];
for (const f of kod) {
  const redove = chist
    .get(f)
    .split('\n')
    .map((r, i) => [r, i + 1])
    .filter(([r]) => {
      const t = r.trim();
      return t !== '' && !/^[})\];,]+$/.test(t);
    });
  for (let i = 0; i + REDA_V_PROZORETSA <= redove.length; i += 1) {
    const par = redove.slice(i, i + REDA_V_PROZORETSA);
    const klyuch = par.map(([r]) => NORMALIZIRAY(r)).join('|');
    if (klyuch.length < NAY_KASIYAT_KLYUCH) continue;
    const tuk = { kade: f, red: par[0][1] };
    const veche = skeleti.get(klyuch);
    if (veche === undefined) {
      skeleti.set(klyuch, tuk);
      continue;
    }
    strukturni.push({ tuk, veche });
  }
}

// СЛИВАНЕ на съседните прозорци · един блок = една находка
let posledna = null;
for (const { tuk, veche } of strukturni) {
  if (
    posledna !== null &&
    posledna.kade === tuk.kade &&
    tuk.red - posledna.red <= REDA_V_PROZORETSA
  ) {
    posledna = tuk;
    continue;
  }
  posledna = tuk;
  nahodka(
    '8б · дублирано по структура',
    tuk.kade,
    tuk.red,
    `същата ФОРМА като ${izravni(relative(KOREN, veche.kade))}:${veche.red} · различни имена`,
  );
}
vidyal('8б · дублирано по структура', kod.length);

// ── 9 · ПЪТ ДО HTML ИЗВЪН ВРАТАТА ─────────────────────────────────────────

/**
 * ЕДНА ВРАТА, И ТЯ Е ЗАПЕЧАТАНА (резен 6м · ADR-019).
 *
 * Дотук екранът се пазеше с НАВИК: една екранираща функция, викана на 185
 * места, и всяко от тях трябваше да бъде написано. Едно забравено — и чуждо име
 * от негов `.xlsx` става изпълним код, а един изпълним ред тук чете ЦЕЛИЯ
 * Журнал и пише през Портата.
 *
 * Сега пътят е ЕДИН: `app/reshetka/shablon.ts`. Този обход пази да остане един.
 *
 * ═══ ЗАЩО ОБХОД, СЛЕД КАТО БРАУЗЪРЪТ ВЕЧЕ ОТКАЗВА ═══
 *
 * Trusted Types отказва при ИЗПЪЛНЕНИЕ, в браузър, който я поддържа. Обходът
 * отказва при СГЛОБЯВАНЕ, навсякъде. Второто е по-евтино: грешката пада преди
 * коммита, а не пред човек, и не зависи от това кой браузър гледа.
 *
 * ═══ КАКВО СЕ БРОИ ═══
 *
 * Присвояване на `innerHTML` или `outerHTML`, `insertAdjacentHTML`,
 * `document.write`, `eval` и `new Function` — навсякъде в `src/` и `app/`
 * ОСВЕН в самата врата. Прагът е НУЛА: изключение не се предвижда, защото
 * вратата приема всичко през `h`, а адрес на скрипт — през `nashSkript`.
 */
const IZVAN_VRATATA = [
  [/\.(?:inner|outer)HTML\s*=/, 'присвояване на HTML извън вратата'],
  [/\binsertAdjacentHTML\s*\(/, 'insertAdjacentHTML · вратата има `dopishi`'],
  [/\bdocument\.write\s*\(/, 'document.write'],
  [/(?<![.\w])eval\s*\(/, 'eval'],
  [/\bnew\s+Function\s*\(/, 'new Function'],
];
const VRATATA = 'app/reshetka/shablon.ts';

let vidyaniVrata = 0;
for (const f of kod) {
  if (izravni(relative(KOREN, f)) === VRATATA) continue;
  vidyaniVrata += 1;
  chist
    .get(f)
    .split('\n')
    .forEach((red, i) => {
      for (const [izraz, duma] of IZVAN_VRATATA) {
        if (izraz.test(red)) nahodka('9 · път до HTML извън вратата', f, i + 1, duma);
      }
    });
}
vidyal('9 · път до HTML извън вратата', vidyaniVrata);

/**
 * ОБХОД 10 · ШУМЪТ · диагностика, която не минава през записа.
 *
 * ═══ ЗАЩО СЕГА, А НЕ ПО-РАНО ═══
 *
 * До 08.09.2026 в `src/` и `app/` нямаше НИТО ЕДИН `console.` — не по правило,
 * а по случайност. Случайността се разваля от първия бърз `console.log`, който
 * някой оставя „само докато оправя това", и оттам нататък прагът вече не може
 * да е нула, без да се чисти.
 *
 * Затова обходът влиза в деня, в който `src/yadro/zapis.ts` даде РЕДОВНИЯ път.
 * Праг нула не е строгост — той е заварено състояние, което се заковава, преди
 * да е започнало да се руши.
 *
 * ═══ КАКВО СЕ БРОИ И ЗАЩО ИМЕННО ТО ═══
 *
 *   · `console.*`  — диагностика, която никой не чете и никой не изхвърля.
 *                    Редовният път е `zapis.ts`: ниво, време, източник, и
 *                    поле за данни, което не приема лични данни (правило 21).
 *   · `alert(`     — отказът се КАЗВА с думи на екрана (правило 12), не с
 *                    прозорец, който спира браузъра и не се превежда.
 *   · `debugger`   — забравен спирач; в пакета той спира чуждия браузър.
 *
 * ═══ ЗАЩО НУЛА ИЗКЛЮЧЕНИЯ ═══
 *
 * Дори `zapis.ts` не пише на конзолата: той държи редовете в пръстен и ги
 * издава като NDJSON, когато някой ги поиска. Тоест няма файл, който да има
 * нужда от изключение — а изключение, добавено „за всеки случай", е точно
 * пролуката, през която се връща шумът.
 *
 * ═══ КАКВО ТОЗИ ОБХОД НЕ ВИЖДА · казано, за да не се вярва повече от нужното ═══
 *
 * Гледа `.ts` в `src/` и `app/`, върху ГОЛИЯ код — коментар с думата `console`
 * не е находка. Не вижда `app/public/sw.js` (той се ПОРАЖДА от
 * `stroezh/pechat-sw.mjs`) и не вижда самите скриптове в `stroezh/`, където
 * `console.log` Е изходът на проверката и трябва да остане.
 */
const SHUM = [
  [/(?<![.\w])console\s*\./, 'console · диагностиката минава през `src/yadro/zapis.ts`'],
  [/(?<![.\w])alert\s*\(/, 'alert · отказът се казва с думи на екрана (правило 12)'],
  [/(?<![.\w])debugger\b/, 'debugger · забравен спирач'],
];

let vidyaniShum = 0;
for (const f of kod) {
  vidyaniShum += 1;
  chist
    .get(f)
    .split('\n')
    .forEach((red, i) => {
      for (const [izraz, duma] of SHUM) {
        if (izraz.test(red)) nahodka('10 · шум · диагностика извън записа', f, i + 1, duma);
      }
    });
}
vidyal('10 · шум · диагностика извън записа', vidyaniShum);

// ── докладът ──────────────────────────────────────────────────────────────

/**
 * ПРАГОВЕТЕ, обяснени поименно.
 *
 * Нула значи „дефект". Число значи „състояние, което се показва": непостроена
 * възможност с тест, домейнен файл, чиито проверки минават през друг файл,
 * или блок, който нарочно стои два пъти с различно обяснение.
 *
 * ЕДИНСТВЕНИЯТ праг над нулата е дублирането, и той е **1** — с име:
 * `razchitane.ts` носи два четеца (по глава и по модел), и всеки завършва с
 * един и същ `catch`, който записва пропуснатия ред с думи. `catch` не се
 * изважда във функция — той е клауза, не израз; а да се слеят двата четеца
 * заради опашката им значи един четец с флаг, което е по-лошо от двата.
 *
 * Прагът е ЗАКОВАН на 1, не „поне 1": втори такъв блок ще върне червено и ще
 * поиска обяснение, вместо да се прибави мълчаливо към този.
 */
const PRAGOVE = {
  '1 · мъртво': 0,
  '2 · излишен export': 0,
  '4 · празно поле': 0,
  '5 · излишен ред': 0,
  '6 · несвързан': 0,
  '8 · дублирано': 1,
  /**
   * 8б е на ДВЕ, и двете с име (резен 6з · ADR-017):
   *
   *   · `formuli/izraz.ts` — нивата на приоритета в парсера (`sabirane` ↔
   *     `umnozhenie`). Те СА една форма, и точно това е правилното: сливането им
   *     значи един четец с флаг за нивото, а флаг вместо две функции е по-лошо;
   *   · `ogledalo/chettsi.ts` — регистърът на четците по тип събитие. Всеки
   *     четец е три реда с една и съща фигура; сливането им значи `switch`, а
   *     регистърът от четци беше избран НАРОЧНО вместо `switch` (ADR-003).
   *
   * Прагът е ЗАКОВАН на 2, не „поне 2": трета такава форма ще върне червено и
   * ще поиска обяснение, вместо да се прибави мълчаливо към тези.
   */
  '8б · дублирано по структура': 2,
  /**
   * НУЛА, и тук няма именувано изключение (резен 6м).
   *
   * Всеки друг праг в този файл има име зад себе си. Тук няма и не бива да има:
   * вратата приема ВСИЧКО — разметка през `h`, адрес на скрипт през
   * `nashSkript`. Който има нужда от втори път, всъщност има нужда да мине
   * през първия.
   */
  '9 · път до HTML извън вратата': 0,
  /**
   * ПРАГ НУЛА · заварено състояние, заковано в деня, в който има редовен път.
   *
   * На 08.09.2026 в `src/` и `app/` има нула `console.`, нула `alert(` и нула
   * `debugger`. Прагът пази това число, а не го постига — разликата е, че
   * тук няма какво да се чисти, има какво да се опази.
   */
  '10 · шум · диагностика извън записа': 0,
};

/**
 * ТОЧНИТЕ ПИНОВЕ · ход 9 (11.09.2026) · обходи 3 и 7 бяха броячи без праг.
 *
 * Брояч без праг не пази нищо: „само тест" можеше да порасне от 32 на 50 и портата
 * оставаше зелена — тоест непостроеното с тест се трупаше мълчаливо, а точно този
 * списък е суровината на правило 30 (построено и невикано е дълг). Пинът е ТОЧЕН:
 * нагоре е находка (ново невикано без ред в дълга), надолу също — за да слезе и
 * пинът с ръка, вместо числото да стои като украса.
 */
const PINOVE_TOCHNI = {
  // 11.09 · ход 9: 32 → 9 (деветте имат ред в дълга: ДЛ-Н3 · Н5 · Н6 · Н7 · Н9 · Н10 · Н11 · Н12)
  // 11.09 · ход Х: 9 → 8 · `ZASHTO_I_NULATA` получи викащ в app/prozorets/smetki.ts (ДЛ-Н9 затворен)
  '3 · само тест': 8,
  // 11.09 · ход 9: 4 → 0 (hash-node.ts · naivno.ts отидоха в tests/, data.ts · zapis.ts получиха викащ)
  '6б · само през барела': 0,
  // 11.09 · ход 9: 47 → 49 — не защото кодът стана по-лош, а защото типовият внос спря да
  // брои за тест (два файла бяха „докоснати" само през `import type` от помощник)
  // 12.09 · ход 97: 49 → 50 · `app/reshetka/izskachasht.ts` е ЕКРАНЕН файл от същия
  // клас като другите четирийсет и девет: вика се (от `sazdavaneto.ts`, от двата
  // прозореца), но се доказва в БРАУЗЪР, не в Node — тестовете тук нямат DOM, а
  // библиотека за DOM иска ADR (правило 9). Покритието му е в прохода.
  // 12.09 · ход 99: 50 → 51 · `app/reshetka/sedmichniyat-prozorets.ts` е от същия
  // екранен клас: вика се от Служители, смятането му живее отделно в
  // `src/smetach/sedmitsata.ts` и Е покрито с тест; тук остава само рисуването.
  // 12.09 · ход 104: 51 → 52 · `app/reshetka/vlizane.ts` е ЕКРАН и се доказва в
  // браузър: проходът минава през него при всяко пускане, защото без него не се
  // стига до нито един прозорец. Тестовете тук нямат DOM, а библиотека иска ADR.
  // 13.09 · ход 109: 52 → 51 · `tests/rezhim.test.ts` внесе и `app/reshetka/pamet-ekran.ts`.
  // 14.09 · 51 → 50 · `src/yadro/dumi.ts` получи свой тест (`tests/dumi.test.ts`).
  // Намерено от обход по МЪРТВОТО, по негово искане (запис 230 т.3): файлът не е
  // мъртъв — той е в ЯДРОТО, вика се отвсякъде, и беше без нито една проверка.
  // Двете функции в него са се местили и са се чупили: `dumiZaGreshka` залепваше
  // латинското име на класа пред българското изречение, а `kolkoMyasto` дойде тук
  // от носителя, защото `app/` няма право да стига дотам.
  // 14.09 · 50 → 51 · `app/reshetka/otmyana.ts` е ЕКРАН (Ctrl+Z · менюто „Отмени" ·
  // „Върни ред") и се доказва в БРАУЗЪР: проход раздел 1в2 изключва ред, връща го
  // от празното, изключва го пак и го връща с клавиша. Чистата половина (КОЕ е
  // следващото за отмяна) е в `src/porta/otmyana.ts` и има свой тест. Същият
  // случай като `vlizane.ts` на 12.09.
  // Пинът СЛИЗА с ръка, защото нагоре и надолу са еднакво находка: число, което
  // не се сваля, утре ще скрие ново невикано под себе си.
  '7 · без тест': 51,
};

const poObhod = new Map();
for (const n of nahodki) poObhod.set(n.obhod, [...(poObhod.get(n.obhod) ?? []), n]);

console.log('\n═══ ЧИСТОТАТА · И101 т.5 ═══\n');
console.log(`  файлове: ${kod.length} код · ${testove.length} тестове`);
console.log(`  изнесени имена: ${imenata.size}\n`);

let cherveno = 0;
for (const obhod of [
  '1 · мъртво',
  '2 · излишен export',
  '3 · само тест',
  '3б · изнесено за теста',
  '4 · празно поле',
  '5 · излишен ред',
  '6 · несвързан',
  '6б · само през барела',
  '7 · без тест',
  '8 · дублирано',
  '8б · дублирано по структура',
  '9 · път до HTML извън вратата',
  '10 · шум · диагностика извън записа',
]) {
  const spisak = poObhod.get(obhod) ?? [];
  const prag = PRAGOVE[obhod];
  const pin = PINOVE_TOCHNI[obhod];
  const previshen =
    (prag !== undefined && spisak.length > prag) || (pin !== undefined && spisak.length !== pin);
  if (previshen) cherveno += 1;
  // Обхватът стои ДО броя находки: нула без обхват значи „не съм гледал".
  const obhvat = vidyani.get(obhod);
  console.log(
    `  ${previshen ? '✗' : '·'} ${obhod}: ${spisak.length}` +
      (prag === undefined ? '' : ` · праг ${prag}`) +
      (pin === undefined
        ? ''
        : ` · пин ${pin}${spisak.length < pin ? ' · слязло е · свали пина с ръка' : spisak.length > pin ? ' · РАСТЕ · новото невикано иска ред в дълга' : ''}`) +
      (obhvat === undefined ? '' : ` · видени ${obhvat.broy} ${obhvat.edinitsa}`),
  );
  // По подразбиране се показват първите 40 · `--vsichki` показва всички.
  // Съкратен списък е за четене; целият — за работа по него.
  const dokade = VSICHKI ? spisak.length : 40;
  for (const n of spisak.slice(0, dokade)) {
    console.log(`      ${n.kade}${n.red ? `:${n.red}` : ''} — ${n.kakvo}`);
  }
  if (spisak.length > dokade) console.log(`      … и още ${spisak.length - dokade}`);
}

// ОБХОД С НУЛЕВ ОБХВАТ Е СЧУПЕН, не чист. Точно това скри обход 7.
const slepi = [...vidyani].filter(([, o]) => o.broy === 0).map(([ime]) => ime);
if (slepi.length > 0) {
  console.log(`\n  ✗ обходи с НУЛЕВ обхват: ${slepi.join(' · ')}`);
  cherveno += slepi.length;
}

console.log('');
if (cherveno > 0) {
  console.log(`НАХОДКИ: ${cherveno} обхода над прага си. Чистотата се БРОИ, не се оценява.\n`);
  process.exit(1);
}
console.log('Чисто: нито един обход над прага си.\n');
