...
describe('кешът за месеца (негово, 05.09 т.2)', () => {
   ...
   */
  /**
   * ДВЕ СВЕРКИ, не една · негово, 13.09 (запис 203) т.3 изброи реда буквално и в
   * него са и двете: „дадено ↔ изтеглено" и „дадено ↔ вкарано по редовете".
   *
   * Втората е ТЪЖДЕСТВО, откакто даденото се смята от същите редове — тя винаги
   * затваря. Слаба сверка, но не безполезна: денят, в който спре да затваря,
   * значи, че даденото и редовете са се разминали. И най-вече — нещо, което той е
   * изброил, не изчезва мълчаливо (правило 12).
   */
  it('даденото се СМЯТА от кеш секциите · и двете сверки стоят на реда', async () => {
    const { iz, zapishi } = await otvori();
    const k0 = keshatNaMeseca(iz.ogledalo(), MESETS, KOGATO);
    expect([k0.dadeno, k0.izvlechenie, k0.vkarano]).toEqual([0, 0, 0]);
    expect(k0.sverki.length).toBe(2);
    ...
    expect(k1.sverki.length).toBe(2);
    expect(k1.sverki[0]?.nared).toBe(false);
    expect(k1.sverki[0]?.razlika).toBe(25000);
    // а втората ЗАТВАРЯ, защото даденото се смята от редовете, срещу които се сверява
    expect(k1.sverki[1]?.kakvo).toContain('вкарано по редовете');
    expect(k1.sverki[1]?.nared).toBe(true);
    expect(k1.sverki[1]?.razlika).toBe(0);
  });
